// app.js (COMPLETO - Bandeja PRO FIX para GitHub Pages)
// - No revienta si falta algún ID
// - Espera DOM listo
// - Maneja DB no lista aún
(() => {
  const $ = (id) => document.getElementById(id);

  // Listener seguro: si el elemento no existe, no hace nada
  function on(id, evt, fn) {
    const el = $(id);
    if (el) el.addEventListener(evt, fn);
  }

  // Esperar DB (window.DB) con reintentos cortos
  async function waitForDB(maxMs = 4000) {
    const t0 = Date.now();
    while (!window.DB || typeof window.DB.all !== "function") {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    navWires.forEach(([id, to]) => on(id, "click", () => showStep(to)));
    ...
    if (form) {
      form.addEventListener("submit", async (e) => {
        ...
        try {
          const okDB = await waitForDB();
          if (!okDB) {
            setMsg("❌ No está listo el almacenamiento (DB). Revisa db.js.", "bad");
            return;
          }
          ...
          const item = {
            ...
            data,
            gps: gpsActual,
            // Guardar Blob en IndexedDB funciona (structured clone)
            foto: fotoBlob
    ...
    if (btnCerrarFoto) btnCerrarFoto.addEventListener("click", closeFoto);
    if (modalFoto) modalFoto.addEventListener("click", (e) => { if (e.target === modalFoto) closeFoto(); });

    if (btnCerrarMapa) btnCerrarMapa.addEventListener("click", closeMapa);
    if (modalMapa) modalMapa.addEventListener("click", (e) => { if (e.target === modalMapa) closeMapa(); });

    if (btnRefrescar) btnRefrescar.addEventListener("click", renderLista);

    if (btnBorrarTodo) btnBorrarTodo.addEventListener("click", async () => {
      const ok = confirm("¿Borrar todo lo guardado local? (solo pruebas)");
      if (!ok) return;

      const okDB = await waitForDB();
      if (!okDB) return setMsg("❌ DB no lista (db.js).", "bad");
    ...
    async function renderLista() {
      ...
      const okDB = await waitForDB();
      if (!okDB) {
        lista.innerHTML = `<div class="sub">DB no lista (revisa db.js).</div>`;
        return;
      }
    ...
    function openFoto(item) {
      ...
      if (btnCerrarFoto) {
        // un solo uso
        const handler = () => {
          btnCerrarFoto.removeEventListener("click", handler);
          closeAndFree();
        };
        btnCerrarFoto.addEventListener("click", handler);
      } else {
        // si no hay botón, al menos libera con click fuera
        setTimeout(() => {
          if (modalFoto) modalFoto.onclick = (e) => { if (e.target === modalFoto) closeAndFree(); };
        }, 0);
      }
    }
